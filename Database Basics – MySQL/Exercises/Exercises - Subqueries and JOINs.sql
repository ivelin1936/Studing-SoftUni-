...
SELECT 
    c2.continent_code, c2.currency_code, c2.currency_usage
FROM
    (SELECT 
        c.continent_code,
            c.currency_code,
            MAX(c.currency_usage) AS currency_usage
    FROM
        (SELECT 
        continent_code,
            currency_code,
    ...
    FROM
        countries
    GROUP BY continent_code , currency_code) AS c
    ...
    FROM
        countries
    GROUP BY continent_code , currency_code) AS c2 ON c1.continent_code = c2.continent_code
WHERE
    c2.currency_usage = c1.currency_usage
